...
jQuery(document).ready(function ($) {
	...
	$(".install").on('click', function (e) {
    	...
    	var ajaxurl = blogerWelcomeObject.ajaxurl;

		$.ajax({
			type: 'POST',
			url: ajaxurl,
			data: {
				action: 'bloger_plugin_installer',
				plugin: plugin,
				plugin_file: plugin_file,
				nonce: blogerWelcomeObject.admin_nonce,
			},
			success: function(response) {
		   		...
		   		if(response == 'success'){
				   		...
				   		el.html(blogerWelcomeObject.installed_btn);
	...
	$('.install-offline').on('click', function (e) {
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'bloger_plugin_offline_installer',
	...
	$(".activate").on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'bloger_plugin_activation',
	   			plugin: plugin,
	   			nonce: blogerWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
	   		success: function(response) {
		   		if(response){
	...
	$('.activate-offline').on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'bloger_plugin_offline_activation',
	   			plugin: plugin,
	   			nonce: blogerWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
