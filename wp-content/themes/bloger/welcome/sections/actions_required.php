<?php
	...
	if($show_success_notice) :
		...
		<div class="action-tab success">
			<h3><?php printf( 'All Recommended action has been successfully completed.', 'bloger'); ?></h3>
			<a class="button button-primary" href="<?php echo esc_url(admin_url('customize.php')); ?>"><?php echo esc_html__('Customize Theme', 'bloger'); ?></a>
		</div>
		<?php
	else :
			...
			if($plugin['bundled'] == false) {
				...
				if(!isset($info->errors)) :
					...
					<div class="action-tab warning">
						<h3><?php printf( esc_html__("Install : %s Plugin", 'bloger'), $info->name ); ?></h3>
						<p><?php esc_html_e('Install Contact Form 7 to add the contact forms.', 'bloger'); ?></p>
				...
				?>
				<div class="action-tab warning">
					<h3><?php printf( esc_html__("Install : %s Plugin", 'bloger'), $plugin['name'] ); ?></h3>
					<p><?php esc_html_e('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'bloger'); ?></p>
