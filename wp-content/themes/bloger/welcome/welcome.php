<?php
	if(!class_exists('Bloger_Welcome')) :

		class Bloger_Welcome {
			...
			public function __construct() {
				...
				$this->req_plugins = array(
					...
					'instant-demo-importer' => array(
						...
						'name' => __('Instant Demo Importer', 'bloger'),
						...
						'info' => __('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'bloger'),
					),
				...
				$this->tab_sections = array(
					'getting_started' => __('Getting Started', 'bloger'),
					'recommended_plugins' => __('Recommended Plugins', 'bloger'),
					'import_demo' => __('Import Demo', 'bloger'),
					'support' => __('Support', 'bloger'),
				);
				...
				add_action( 'load-themes.php', array( $this, 'bloger_activation_admin_notice' ) );

				/* Create a Welcome Page */
				add_action( 'admin_menu', array( $this, 'bloger_welcome_register_menu' ) );

				/* Enqueue Styles & Scripts for Welcome Page */
				add_action( 'admin_enqueue_scripts', array( $this, 'bloger_welcome_styles_and_scripts' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_bloger_plugin_installer', array( $this, 'bloger_plugin_installer_callback' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_bloger_plugin_offline_installer', array( $this, 'bloger_plugin_offline_installer_callback' ) );

				/** Plugin Activation Ajax **/
				add_action( 'wp_ajax_bloger_plugin_activation', array( $this, 'bloger_plugin_activation_callback' ) );

				/** Plugin Activation Ajax (Offline) **/
				add_action( 'wp_ajax_bloger_plugin_offline_activation', array( $this, 'bloger_plugin_offline_activation_callback' ) );
			...
			public function get_required_plugin_notification() {
				...
				$bloger_plugin_installed_notif = get_option('bloger_plugin_installed_notif');
				...
				update_option('bloger_plugin_installed_notif', $notif_counter);
				return $notif_counter;
			}

			/** Welcome Message Notification on Theme Activation **/
			public function bloger_activation_admin_notice() {
				global $pagenow;

				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					?>
					<div class="notice notice-success is-dismissible">
						<p><?php printf( __( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with %1$s.', 'bloger' ), $this->theme_name, admin_url( 'themes.php?page=bloger-welcome' )  ); ?></p>
						<p><a class="button" href="<?php echo admin_url( 'themes.php?page=bloger-welcome' ) ?>"><?php _e( 'Lets Get Started', 'bloger' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function bloger_welcome_register_menu() {
				$action_count = get_option('bloger_plugin_installed_notif');
				$title        = $action_count > 0 ? esc_html__( 'Welcome', 'bloger' ) : esc_html__( 'Welcome', 'bloger' );
				add_theme_page( 'Welcome', $title , 'edit_theme_options', 'bloger-welcome', array( $this, 'bloger_welcome_screen' ));
			}

			/** Welcome Page **/
			public function bloger_welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php printf( esc_html__( 'Welcome to %s - Version %s', 'bloger' ), $this->theme_name, $this->theme_version ); ?></h1>
					<div class="about-text"><?php printf( esc_html__( '%s is a simple, clean and minimal blog theme suitable for personal or professional blogs, company or organization blog websites, news, magazine websites. The theme is fully customizable and contains all the features needed for a complete blog website. It is Fully responsive, bbPress compatible, translation ready, cross-browser compatible, SEO friendly, RTL ready.', 'bloger' ), $this->theme_name ); ?></div>
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo admin_url('themes.php?page=bloger-welcome&section='.$id); ?>" class="<?php echo $nav_class; ?>" >
			...
			public function bloger_welcome_styles_and_scripts() {
				...
				wp_localize_script( 'accesspress-basic-welcome-screen', 'blogerWelcomeObject', array(
					'admin_nonce'	=> wp_create_nonce('bloger_plugin_installer_nonce'),
					'activate_nonce'	=> wp_create_nonce('bloger_plugin_activate_nonce'),
					'ajaxurl'		=> esc_url( admin_url( 'admin-ajax.php' ) ),
					'activate_btn' => __('Activate', 'bloger'),
					'installed_btn' => __('Activated', 'bloger'),
					'demo_installing' => __('Installing Demo', 'bloger'),
					'demo_installed' => __('Demo Installed', 'bloger'),
					'demo_confirm' => __('Are you sure to import demo content ?', 'bloger'),
				) );
			}

			/** Plugin API **/
			public function bloger_call_plugin_api( $plugin ) {
			...
			/** Check For Icon **/
			public function bloger_check_for_icon( $arr ) {
				if ( ! empty( $arr['svg'] ) ) {
			...
			public function bloger_plugin_active($plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
				$status = 'install';
			...
			public function bloger_plugin_generate_url($status, $plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
			...
			/* ========== Plugin Installation Ajax =========== */
			public function bloger_plugin_installer_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to install plugins on this site.', 'bloger' ) );
				...
				if (! wp_verify_nonce( $nonce, 'bloger_plugin_installer_nonce' ))
					wp_die( __( 'Error - unable to verify nonce, please try again.', 'bloger') );
				...
				// Get Plugin Info
				$api = $this->bloger_call_plugin_api($plugin);
			...
			public function bloger_plugin_offline_installer_callback() {
			...
			public function bloger_plugin_offline_activation_callback() {
			...
			/** Plugin Activation Ajax **/
			public function bloger_plugin_activation_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to activate plugins on this site.', 'bloger' ) );

				$nonce = $_POST["nonce"];
				$plugin = $_POST["plugin"];

				// Check our nonce, if they don't match then bounce!
				if (! wp_verify_nonce( $nonce, 'bloger_plugin_activate_nonce' ))
					die( __( 'Error - unable to verify nonce, please try again.', 'bloger' ) );
				...
				// Get Plugin Info
				$api = $this->bloger_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'bloger');
				}
			...
			public static function get_plugin_file( $plugin_slug ) {
	      	...
	      	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=bloger-welcome&section=import_demo'),'bloger-file-installation');
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'bloger' => array(
				'title' => __('Bloger Demo', 'bloger'),
				'name' => 'bloger',
				'screenshot' => get_template_directory_uri().'/welcome/demos/bloger/screen.png',
				'home_page' => '',
				'menus' => array(
					'Menu 1' => 'primary'
				)
